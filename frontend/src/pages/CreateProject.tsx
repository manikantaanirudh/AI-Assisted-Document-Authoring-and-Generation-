...
import { projectService, DocumentType, SectionCreate } from '../services/projects';
...
const CreateProject: React.FC = () => {
  ...
  const handleRemoveSection = (index: number) => {
    const newSections = sections.filter((_, i) => i !== index).map((s, i) => ({
      ...s,
      order_index: i
    }));
    ...
    <div className="min-h-screen bg-gradient-dark">
      ...
      <main className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="animate-fadeIn">
          ...
          {/* Glass Card Form */}
          <form onSubmit={handleSubmit} className="glass-card p-8 space-y-8 animate-fadeInUp">
            {error && (
            ...
            <div>
              <label htmlFor="docType" className="block text-sm font-medium text-silver mb-2">
                Document Type *
              </label>
              <select
                id="docType"
                ...
                required
              >
                <option value="docx" className="bg-black text-white">Word Document (.docx)</option>
                <option value="pptx" className="bg-black text-white">PowerPoint Presentation (.pptx)</option>
              </select>
            </div>

            <div>
              <label htmlFor="topic" className="block text-sm font-medium text-silver mb-2">
                Main Topic *
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              <div className="space-y-3 mb-4">
                {sections.map((section, index) => (
                  <div key={index} className="flex items-center space-x-2 glass-card p-3">
                    <input
                      type="text"
                      value={section.title}
                      onChange={(e) => {
