...
const ProjectView: React.FC = () => {
  ...
  const [feedbackStatus, setFeedbackStatus] = useState<Record<string, { liked: boolean | null; message: string }>>({});
  ...
  const handleGenerate = async (sectionId: string) => {
    ...
    try {
      const response = await generationService.generateSection({
  ...
  const handleFeedback = async (sectionId: string, liked: boolean) => {
    ...
    setFeedbackStatus({ ...feedbackStatus, [sectionId]: { liked: null, message: '' } });
    ...
    try {
      ...
      setFeedbackStatus({
        ...
        [sectionId]: {
          liked,
          message: liked ? '👍 Thank you for your feedback!' : '👎 Thanks, we\'ll improve this!'
        }
      });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-dark flex items-center justify-center">
        <div className="text-lg text-silver animate-slowPulse">Loading project...</div>
      </div>
    );
  }
      ...
      <main className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="animate-fadeIn">
          ...
          <div className="glass-card p-8 mb-8 animate-fadeInUp">
            <h2 className="text-display-3 font-display text-white mb-4 tracking-tight">{project.title}</h2>
            ...
            <div className="flex items-center space-x-4">
              ...
              <span className="text-sm text-silver">
                {project.sections.length} {project.doc_type === 'docx' ? 'sections' : 'slides'}
              </span>
            </div>
          </div>
          ...
          {project.sections.length === 0 ? (
            <div className="glass-card p-12 text-center">
              <p className="text-silver">No sections yet. Configure your project first.</p>
            </div>
          ) : (
              ...
              {project.sections.map((section, index) => (
                <SectionCard
                  ...
                  onRefinementPromptChange={(value) =>
                    setRefinementPrompts({ ...refinementPrompts, [section.id]: value })
                  }
...
const SectionCard: React.FC<SectionCardProps> = ({
  ...
  return (
    <div className="section-card animate-fadeInUp" style={{ animationDelay: `${index * 0.1}s` }}>
        ...
        {section.content && (
          <div className="border-t border-white/5 pt-6">
            ...
            {feedbackStatus?.message && (
              <div className={`mb-3 px-4 py-2 rounded-lg text-sm backdrop-blur-sm ${
